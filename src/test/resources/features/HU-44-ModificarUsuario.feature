Feature: Modificar usuario
  Como usuario Quiero modificar un usuario Para continuar su gestión.

  Background: Ingreso a modificar usuario
  ...
  Scenario Outline: 4.No ingresar información en los campos requeridos
    When  En modificar usuario borra la información del <campo_requerido>
  ...
  Scenario Outline: 5.Se habilita si o no botón de guardar
    When  En modificar usuario ingresa o selecciona informacion en los campos <tipo_documento> <documento> <nombre> <apellidos> <telefono> y <rol>
    Then  En modificar usuario sistema presenta <habilitado> el boton de guardar
  ...
  Scenario Outline: 6.Ingresar informacion en los campos de crear usuario
    When En modificar usuario Ingresa o selecciona en el <campo> del formulario un <valor>
    Then En modificar usuario El sistema muestra el <resultado>
  ...
  Scenario: Carga de foto
    Given En modificar usuario da clic enlace a Agregar foto
    When En modificar usuario sube una foto
    Then En modificar Se muestra la foto cargada

  Scenario: Opciones con foto cargada
    Given En modificar usuario da clic enlace a Agregar foto
    And En modificar usuario sube una foto
    And En modificar Se muestra la foto cargada
  ...
  Scenario Outline: Eliminar la foto
    ...
    And En modificar usuario sube una foto
    And En modificar Se muestra la foto cargada
    And En modificar usuario se clic en la foto
    ...
    And En modificar usuario selecciona la opcion de <opcion> eliminar
    Then En modificar usuario se tiene la <accion> de eliminar
  ...
  Scenario Outline:  9.Modificación de usuario
    When  En modificar usuario ingresa o selecciona informacion en los campos <tipo_documento> <documento> <nombre> <apellidos> <telefono> y <rol>
    And En modificar usuario muestra en el campo usuario el <resultado>
    And   En modificar usuario da clic en boton guardar
    Then  En modificar usuario sistema presenta el <mensaje> de exito o error
